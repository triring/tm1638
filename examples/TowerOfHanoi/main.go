...
func main() {
	...
	moveDisk := func(from, to string) {
		...
		textHanoi[5] = diskFont[towerA]
		textHanoi[6] = diskFont[towerB]
		textHanoi[7] = diskFont[towerC]
		TM1638.Disp7SEGs(textHanoi)
	...
	// 1. 変数を先に宣言しておく
	var hanoi func(n int, from, to, via string) 
	// 2. 変数に関数を代入する
	// n: 円盤の枚数
	// from: 移動元の柱
	// to: 移動先の柱
	// via: 経由する柱
	hanoi = func(n int, from, to, via string) {
		if n == 1 {
